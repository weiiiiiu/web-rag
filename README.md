...
4. **网络要求**：需要能够访问 MinerU API、GitHub API 和阿里云 API（如使用知识库功能）
5. **费用**：MinerU 和阿里云百炼服务按量计费，请注意成本控制
6. **路径配置**：使用相对路径，项目可移植性更好
...
```yaml
file:
  tmp-dir: ./tmp           # 临时目录（相对路径）
  result-dir: ./results    # 结果目录（相对路径）
```
